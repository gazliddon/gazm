...
impl GazmParser {
    ...
    pub fn parse_assembly(cpu_kind: CpuKind, input: TSpan) -> PResult<Node> {
        let (rest, nodes) = cpu_dispatch!(parse_multi, cpu_kind, input)?;
        if nodes.len() == 1 {
    ...
    pub fn parse_command_args(k: CommandKind, args: TSpan) -> PResult<Node> {
        ...
        match k {
            ...
            ZeroFill => Self::parse_zero_fills(args),
            Fill => Self::parse_fill(args),
            EmitBytes => Self::parse_emit_bytes(args),
            EmitWords => Self::parse_emit_words(args),
            ...
            ReserveLongs => Self::parse_reserve_longs(args),
